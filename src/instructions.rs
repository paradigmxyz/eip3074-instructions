...
pub fn boxed_instructions<'a, EXT: 'a, DB: Database + 'a>(
    ...
    let boxed_auth_instruction = Box::new(
        move |interpreter: &mut Interpreter, evm: &mut Context<EXT, DB>| {
...
mod tests {
    ...
    fn setup_auth_stack(stack: &mut Stack, authority: Address) {
        ...
        stack
            .push_b256(B256::left_padding_from(authority.as_slice()))
            .unwrap();
    }
    ...
    fn setup_authcall_stack(stack: &mut Stack, gas: u64, to: Address, value: u64) {
        ...
        stack
            .push_b256(B256::left_padding_from(to.as_slice()))
            .unwrap();
    ...
    fn test_auth_instruction_happy_path() {
        ...
        assert_eq!(
            context.get(AUTHORIZED_VAR_NAME).unwrap(),
            authority.to_vec()
        );
    }
    ...
    fn test_auth_instruction_warm_authority() {
        ...
        evm.evm
            .journaled_state
            .state
            .insert(authority, Account::default());
